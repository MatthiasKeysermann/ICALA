...
public class Monitor extends Simulation {
	...
	public Monitor() {
		...
		spCycleTime.setModel(new SpinnerNumberModel((int) CYCLE_TIME_DEFAULT, 0, 10000, 100));
		...
		fcSaveLoad.setSelectedFile(new File(System.getProperty("user.dir") + "/TOSAM.xml"));
				...
				if (cbUnitsUpdate.isSelected()) {
					...
					synchronized (units) {
						for (Unit unit : units) {
							...
							rowData[2] = unit.getLoad();
							rowData[3] = unit.getActivation();
				...
				if (cbAssociationsUpdate.isSelected()) {
					...
					synchronized (associations) {
						for (Association association : associations) {
							...
							rowData[3] = association.getWeight();
							rowData[4] = association.getSignal();
	...
	private class TableUnits extends JTable {
		...
		public Component prepareRenderer(TableCellRenderer renderer, int row, int column) {
			...
			double activation = (Double) this.getValueAt(row, 3);
			double activationMin = Unit.ACTIVATION_MIN;
			double activationMax = Unit.ACTIVATION_MAX;
			if (activation < activationMin) {
				activation = activationMin;
			} else if (activation > activationMax) {
				activation = activationMax;
			}
			double range = activationMax - activationMin;
			int grey = (int) Math.round((activation - activationMin) / range * 155) + 100;
			if (grey < 0)
				grey = 0;
			if (grey > 255)
				grey = 255;
	...
	private class TableAssociations extends JTable {
		...
		public Component prepareRenderer(TableCellRenderer renderer, int row, int column) {
			...
			double weight = (Double) this.getValueAt(row, 3);
			double weightMin = Association.WEIGHT_MIN;
			double weightMax = Association.WEIGHT_MAX;
			if (weight < weightMin) {
				weight = weightMin;
			...
			int grey = (int) Math.round((weight - weightMin) / range * 155) + 100;
			if (grey < 0)
				grey = 0;
			if (grey > 255)
				grey = 255;
