...
public class NAOBumperRight extends InteractorUDP {

	private static final String NAME = "NAOBumperRight";
	...
	public NAOBumperRight() {
		...
		// create panel
		panel = new JPanel();
		panel.setBorder(BorderFactory.createTitledBorder("Bumper Right"));
		frame.add(panel);
	...
	protected double[] readInput() {
		...
		Variant varBumperRight = memory.getData("RightBumperPressed");
		double bumperRight = (double) varBumperRight.toFloat();

		// create pattern
		double[] pattern = new double[INPUT_DIM];
		pattern[0] = 0.5;
		pattern[1] = bumperRight;
	...
	protected void updateUI() {
		...
		synchronized (panel) {
			...
			String strNotPressed = "Not Pressed";
			textWidth = metrics.stringWidth(strNotPressed);
			g.drawString(strNotPressed, (width - textWidth) / 2, (int) Math.round(height * 0.1));
			String strPressed = "Pressed";
			textWidth = metrics.stringWidth(strPressed);
			g.drawString(strPressed, (width - textWidth) / 2, (int) Math.round(height * 0.9));
	...
	public static void main(String[] args) {
		NAOBumperRight interactor = new NAOBumperRight();
		interactor.run();
	}
